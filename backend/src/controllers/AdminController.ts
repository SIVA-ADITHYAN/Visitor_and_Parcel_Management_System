...
export class AdminController {
  static async getDashboardStats(req: AuthRequest, res: Response) {
    try {
      ...
      res.json({
        visitors: (visitorStats as any)[0],
        parcels: (parcelStats as any)[0],
  ...
  static async getQuickActions(req: AuthRequest, res: Response) {
    try {
      const connection = await createConnection();
  ...
  static async bulkStatusUpdate(req: AuthRequest, res: Response) {
    try {
      ...
      await connection.execute(
        `UPDATE visitors_parcels SET status = ?, notes = ?, updated_at = NOW() WHERE id IN (${placeholders})`,
        [status, notes, ...ids]
      );
